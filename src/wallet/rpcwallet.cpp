...
void EnsureWalletIsUnlocked(const CWallet* pwallet)
{
    ...
    if (IsRubixWallet(pwallet)
        && GetRubixWallet(pwallet)->fUnlockForStakingOnly)
...
static UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"getnewaddress",
                ...
                RPCResult{
            "\"address\"    (string) The new Rubix address\n"
                },
                RPCExamples{
    ...
    if (IsRubixWallet(pwallet)) {
        ...
        CPubKey newKey;
        CHDWallet *phdw = GetRubixWallet(pwallet);
        {
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            RPCHelpMan{"getrawchangeaddress",
                "\nReturns a new Rubix address, for receiving change.\n"
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The Rubix address to be associated with a label."},
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 10)
        throw std::runtime_error(
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Rubix address to send to."},
            ...
            "                             The recipient will receive less Rubix than you enter in the amount field."},
...
static UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The Rubix address\n"
        ...
        throw std::runtime_error(
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Rubix address to use for the private key."},
...
static UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Rubix address for transactions."},
    ...
    for (const std::pair<const uint256, CWalletTx>& pairWtx : pwallet->mapWallet) {
        ...
        if ((!fRubixWallet && wtx.IsCoinBase()) || !locked_chain->checkFinalTx(*wtx.tx)) {
            continue;
        }
...
static UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    const RPCHelpMan help{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The Rubix address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Rubix address: ") + name_);
        }
...
static UniValue addmultisigaddress(const JSONRPCRequest& request)
{
        ...
        std::string msg =
            RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of Rubix addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "Rubix address or hex-encoded public key"},
                        },
                        },
    ...
    bool fbech32 = fRubixMode && request.params.size() > 3 ? request.params[3].get_bool() : false;
    bool f256Hash = fRubixMode && request.params.size() > 4 ? request.params[4].get_bool() : false;
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain(locked_chain) >= nMinDepth) {
        for (const COutputEntry &r : listReceived) {
            ...
            if (fRubixWallet
                ...
                if (GetRubixWallet(pwallet)->GetStealthLinked(idK, sx)) {
            ...
            if (wtx.IsCoinBase()) {
                ...
                if (wtx.IsImmatureCoinBase(locked_chain)) {
                    ...
                    entry.pushKV("category", (fRubixMode ? "coinbase" : "generate"));
                }
            } else {
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"address\",    (string) The Rubix address of the transaction.\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"details\" : [\n"
            "    {\n"
            "      \"address\" : \"address\",          (string) The Rubix address involved in the transaction\n"
            "      \"category\" :                      (string) The transaction category.\n"
            "                   \"send\"                  Transactions sent.\n"
            "                   \"receive\"               Non-coinbase transactions received.\n"
            "                   \"generate\"              Coinbase transactions received with more than 100 confirmations.\n"
...
static UniValue abandontransaction(const JSONRPCRequest& request)
{
    ...
    if (!pwallet->mapWallet.count(hash)) {
        if (!IsRubixWallet(pwallet) || !GetRubixWallet(pwallet)->HaveTransaction(hash)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid or non-wallet transaction id");
        }
    }
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3) {
        throw std::runtime_error(
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending Rubix\n"
            "\nNote:\n"
...
static UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    int nTxCount = (int)pwallet->mapWallet.size() + (fRubixWallet ? (int)((CHDWallet*)pwallet)->mapRecords.size() : 0);
    obj.pushKV("txcount",       (int)nTxCount);

    CKeyID seed_id;
    if (IsRubixWallet(pwallet)) {
        CHDWallet *pwhd = GetRubixWallet(pwallet);
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"loadwallet",
                ...
                "\nNote that all wallet command-line options used when starting Rubixd will be"
...
static UniValue createwallet(const JSONRPCRequest& request)
{
    ...
    if (fRubixMode && !GetRubixWallet(wallet.get())->Initialise()) {
    ...
    if (!passphrase.empty() && !(flags & WALLET_FLAG_DISABLE_PRIVATE_KEYS)) {
        ...
        if (!create_blank) {
            ...
            if (fRubixMode && !!GetRubixWallet(wallet.get())->MakeDefaultAccount()) {
...
static UniValue listunspent(const JSONRPCRequest& request)
{
        ...
        throw std::runtime_error(
            RPCHelpMan{"listunspent",
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of Rubix addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "Rubix address"},
                        },
                    },
            ...
            "    \"coldstaking_address\"  : \"address\" (string) the Rubix address this output must stake on\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Rubix address: ") + input.get_str());
            }
    ...
    std::vector<COutput> vecOutputs;
    {
        ...
        auto locked_chain = pwallet->chain().lock();
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableCoins(*locked_chain, vecOutputs, !include_unsafe, &cctl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth, fIncludeImmature);
    }
...
void FundTransaction(CWallet* const pwallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, UniValue options)
{
    ...
    size_t nOutputs = IsRubixWallet(pwallet) ? tx.vpout.size() : tx.vout.size();
    if (nOutputs == 0)
    ...
    const RPCHelpMan help{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The Rubix address to receive the change"},
...
static UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (totalFee > 0 || IsRubixWallet(pwallet)) {
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given Rubix address. Some information requires the address\n"
            "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The Rubix address to get the information of."},
                },
                RPCResult{
            "{\n"
            "  \"address\" : \"address\",        (string) The Rubix address validated\n"
...
UniValue sethdseed(const JSONRPCRequest& request)
{
    ...
    if (IsRubixWallet(pwallet))
        throw JSONRPCError(RPC_WALLET_ERROR, "Not necessary in Rubix mode.");
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the Rubix address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The Rubix address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
